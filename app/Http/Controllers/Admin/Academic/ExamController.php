...
class ExamController extends Controller
{
    ...
    public function questionPaper(Exam $exam)
    {
        $serialNo = 1;
        $exam->load('questions');
        $questionLibrary = QuestionLibrary::where('is_active', true)
            ->orderBy('topic')
            ->orderBy('id')
            ->get()
            ->groupBy('topic');
    ...
    public function storeQuestion(QuestionFormRequest $request, Exam $exam)
    {
        try {
    ...
    public function storeFromLibrary(Request $request, Exam $exam)
    {
        $request->validate([
        ...
        try {
            $libraries = QuestionLibrary::whereIn('id', $request->library_question_ids)
                                        ->where('is_active', true)
                                        ->get();
    ...
    public function updateQuestion(QuestionFormRequest $request, Exam $exam, Question $question)
    {
        try {
            if ($request->hasFile('question_figure')) {
                $uploadPath = 'storage/question_figure/';
