...
class ExamRuleController extends Controller
{
    ...
    {
        $serialNo = 1;
        $ruleList = ExamRule::orderBy('id', 'ASC')->where('deleted_at', NULL)->get();
    ...
    public function edit(ExamRule $examRule)
    {
        $serialNo    = 1;
        $ruleList    = ExamRule::orderBy('id', 'ASC')->where('deleted_at', NULL)->get();
    ...
    public function update(ExamRuleFormRequest $request, ExamRule $examRule)
    {
        try {
            $examRule->update([
                'type'        => $request->type,
