...
class TechQuestionLibraryController extends Controller
{
    public function index()
    {
        $serialNo = 1;
        $questionList = AcaQuestionLibrary::orderBy('id', 'ASC')->where('deleted_at', NULL)->get();
        return view('teacher.modules.questionsLibrary.index', compact('serialNo', 'questionList'));
    }

    public function create()
    {
        $courseList = AcaCourse::orderBy('id', 'ASC')->where('deleted_at', NULL)->get();
        return view('teacher.modules.questionsLibrary.form', compact('courseList'));
    }
    ...
    public function store(QuestionLibraryFormRequest $request)
    {
        try {
            ...
            if ($request->hasFile('question_figure')) {
                $uploadPath = 'assets/storage/question_figure/library/';
            ...
            return redirect()->route('teacher.questions.library.index')->with('success', 'Question has been created successfully to library!');
    ...
    {
        $courseList = AcaCourse::orderBy('id', 'ASC')->where('deleted_at', NULL)->get();
        return view('teacher.modules.questionsLibrary.form', compact('questionLib', 'courseList'));
    }
    ...
    public function update(QuestionLibraryFormRequest $request, AcaQuestionLibrary $questionLib)
    {
        try {
            ...
            return redirect()->route('teacher.questions.library.index')->with('success', 'Question has been updated successfully to library!');
        } catch (\Throwable $exception) {
    ...
    public function destroy(AcaQuestionLibrary $questionLib)
    {
        ...
        return redirect()
            ->route('teacher.questions.library.index')
