...
class TechStudentController extends Controller
{
    ...
    {
        $serialNo = 1;
        $studentList = Student::where('aca_created_by', auth()->id())->orderBy('id', 'ASC')->get();
        return view('teacher.modules.students.index', compact('studentList', 'serialNo'));
    }

    public function create()
    {
        return view('teacher.modules.students.form');
    }
    ...
    public function store(StudentFormRequest $request)
    {
        try {
            ...
            return redirect()->route('teacher.students.index')->with('success', 'Student has been created successfully!');
    ...
    {
        return view('teacher.modules.students.form', compact('student'));
    }
    ...
    {
        $student->load('info');
        return view('teacher.modules.students.profile.form', compact('student'));
    }
    ...
    public function studentProfileStore(Request $request, Student $student)
    {
        ...
        $data['aca_created_by'] = auth('admin')->id();
        $data['aca_updated_by'] = auth('admin')->id();
        ...
        return redirect()->route('teacher.students.profile', $student->id)
