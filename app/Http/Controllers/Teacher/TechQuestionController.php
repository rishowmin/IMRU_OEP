...
class TechQuestionController extends Controller
{
    ...
    {
        $serialNo = 1;
        $questionList = AcaQuestion::orderBy('id', 'ASC')->where('aca_created_by', auth()->id())->where('deleted_at', NULL)->get();
        return view('teacher.modules.questions.index', compact('serialNo', 'questionList'));
    }

    public function create()
    {
        $examList = AcaExam::orderBy('id', 'ASC')->where('aca_created_by', auth()->id())->where('deleted_at', NULL)->get();
        $courseList = AcaCourse::orderBy('id', 'ASC')->where('aca_created_by', auth()->id())->where('deleted_at', NULL)->get();
        return view('teacher.modules.questions.form', compact('examList', 'courseList'));
    }
    ...
    public function store(QuestionFormRequest $request)
    {
        try {
            ...
            return redirect()->route('teacher.questions.index')->with('success', 'Question has been created successfully!');
    ...
    public function edit(AcaQuestion $question)
    {
        $examList = AcaExam::orderBy('id', 'ASC')->where('aca_created_by', auth()->id())->where('deleted_at', NULL)->get();
        $courseList = AcaCourse::orderBy('id', 'ASC')->where('aca_created_by', auth()->id())->where('deleted_at', NULL)->get();
        return view('teacher.modules.questions.form', compact('question', 'examList', 'courseList'));
    }
    ...
    public function update(QuestionFormRequest $request, AcaQuestion $question)
    {
        try {
            ...
            return redirect()->route('teacher.questions.index')->with('success', 'Question has been updated successfully!');
        } catch (\Throwable $exception) {
